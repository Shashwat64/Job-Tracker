...
export const uploadResume = async (req, res) => {
    ...
    try {
        ...
        const dbResult = await pool.query(`INSERT INTO resumes (user_id, name, file_url) 
       VALUES ($1, $2, $3) 
       RETURNING *`, [userId, name, fileUrl]);
        res.status(201).json({
...
export const getResumes = async (req, res) => {
    const userId = req.userId;
    try {
        const result = await pool.query(`SELECT * FROM resumes WHERE user_id = $1 ORDER BY created_at DESC`, [userId]);
...
export const deleteResume = async (req, res) => {
    ...
    try {
        const result = await pool.query(`DELETE FROM resumes WHERE id = $1 AND user_id = $2 RETURNING *`, [resumeId, userId]);
